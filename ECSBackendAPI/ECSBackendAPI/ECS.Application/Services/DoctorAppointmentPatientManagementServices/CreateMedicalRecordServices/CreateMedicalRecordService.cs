...
namespace ECS.Application.Services.DoctorAppointmentPatientManagementServices.CreateMedicalRecordServices
{
    ...
    public class CreateMedicalRecordService : ICreateMedicalRecordService
    {
        ...
        private void CheckMedicalRecordExists(ExecutionState state)
        {
            ...
            var existingRecord = _medicalRecordRepository
            ...
            if (state.IsMedicalRecordExists)
            {
                state.HasError = true;
        ...
        {
            if (state.HasError || state.Appointment == null || state.DoctorProfile == null || state.PatientProfile == null)
                return;
            ...
            try
            {
                ...
                var medicalRecord = new MedicalRecord
                {
                    ...
                    IllnessDayNumber = request.IllnessDayNumber,
                    
                    // II. Hỏi bệnh
                    MedicalHistory = request.MedicalHistory,
                    PersonalHistoryEye = request.PersonalHistoryEye,
                    PersonalHistorySystemic = request.PersonalHistorySystemic,
                    FamilyHistory = request.FamilyHistory,
                    
                    // Vital Signs
                    VitalPulse = request.VitalPulse,
                    VitalTemperature = request.VitalTemperature,
                    VitalBloodPressure = request.VitalBloodPressure,
                    VitalRespiratoryRate = request.VitalRespiratoryRate,
                    VitalWeightKg = request.VitalWeightKg,
                    
                    // Systemic Exam (serialize to JSON)
                    SystemicExam = request.SystemicExam != null ? System.Text.Json.JsonSerializer.Serialize(request.SystemicExam) : null,
                    
                    // VI. Chẩn đoán
                    DiagnosisMain = request.DiagnosisMain,
                    DiagnosisComorbid = request.DiagnosisComorbid,
                    DiagnosisDifferential = request.DiagnosisDifferential,
                ...
                // Create eye examinations for both eyes
                await CreateEyeExaminationsAsync(_context, medicalRecord.Id, request);

                // Create subspecialty records based on record type
        ...
        {
            if (request.Prescription == null || request.PrescriptionItems == null || request.PrescriptionItems.Count == 0)
                return;
        ...
        private async Task UpdateStatusesAsync(ExecutionState state)
        {
            ...
            try
            {
                ...
                var queue = await _context.Queues
                    .FirstOrDefaultAsync(q => q.AppointmentId == state.AppointmentId);
        ...
        private ApiResponse<CreateMedicalRecordResponse> CreateResponse(ExecutionState state)
        {
            if (state.HasError)
            {
                ...
                return ApiResponse<CreateMedicalRecordResponse>.Fail(errorCode);
            }
            ...
            return ApiResponse<CreateMedicalRecordResponse>.Success(
